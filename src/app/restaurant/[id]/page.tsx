...
export default function RestaurantDetailPage() {
  ...
  const loadRestaurantData = async () => {
    try {
      ...
      // Google Analytics - Restaurant View Event
      try {
        Analytics.viewRestaurant(restaurantId, restaurantData.name);
      } catch (error) {
        console.error('🟡 Analytics restaurant view error (non-blocking):', error);
      }
  ...
  const addToCart = (product: Product) => {
    ...
    try {
      const analyticsProduct = createAnalyticsProduct(product, 1);
      Analytics.viewItem(analyticsProduct);
    } catch (error) {
      console.error('🟡 Analytics product view error (non-blocking):', error);
    }
